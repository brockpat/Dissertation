...
Decomposition for each stock characteristic separately.

Only Implemented for the Baseline Variables
"""
...
#---- DataFrame to store numerical errors (More complicated as in Baseline)
#Get Characteristic names
year = 2002
VarDecomp_df = pd.read_stata(path + "/Output/Variance Decomposition Python/" + f"VarDecomp_{filename}_Restricted_{year}"f"-{month_dict[q]}"f"-{day_dict[q]}.dta")
VarDecomp_df['rdate'] =  pd.to_datetime(VarDecomp_df["rdate"])
Characteristics_Names = [var for var in list(VarDecomp_df.columns) 
                         ...
                         if var not in ['rdate','bin','mgrno','aum','LNrweight','rweight','LNshrout',
                                        'LNprc','LNcfac','unpref', 'permno', 'LNme', 'constant',
                                        'cons']]
del VarDecomp_df

#Set up Dataframe
df_numerical_errors = pd.DataFrame(columns = ['rdate','root1_error']
                                   + ["root2_error_" + var for var in Characteristics_Names] 
                                   + ['root3_error','root4_error','root5_error']
                                   )
del Characteristics_Names
...
for year in range(2002, 2022):
    ...
    #Numerical Errors from Root Finding
    num_errors = []
    ...
    num_errors.append(np.linalg.norm(root1.fun))
    ...
    #Create Dataframe with x_t and x_{t+4} for stocks at time t (LNme dropped as fully endogenous)
    x_list = [item for item in prev_x.columns if item not in ['permno', 'constant', 'LNme']]
    x_list_lead = [item + "_Lead" for item in prev_x.columns if item not in ['permno','constant', 'LNme']]
    
    LNprc_2 = prev_p.copy()
    for i, var in enumerate(x_list):
        print(var)
        Lead_x = prev_x.merge(Lead_VarDecomp_df[['permno'] + [var]].drop_duplicates(subset = 'permno'), 
                              how='left', 
                              on='permno', suffixes=('', '_Lead')
                              )
    
        #Update x_t with values from t+4 if they exist.
        Lead_x[var] = np.where(pd.notnull(Lead_x[var + "_Lead"]), Lead_x[var + "_Lead"], Lead_x[var])
        #Delete auxiliary column
        Lead_x.drop(var + "_Lead",axis = 1, inplace = True)
        ...
        num_errors.append(np.linalg.norm(root2.fun))
        #Save Results
        LNprc_2['LNprc_2_' + var] = root2.x
    LNprc_2.drop('LNprc',axis = 1, inplace = True)
    ...
    #Solve for the Market Clearing Price
    root3 = solve_MarketClearing(LNprc_2[LNprc_2.columns[-1]].to_numpy(), 
    ...
    num_errors.append(np.linalg.norm(root3.fun))
    ...
    num_errors.append(np.linalg.norm(root4.fun))
    ...
    num_errors.append(np.linalg.norm(root5.fun))
    ...
    for i, var in enumerate(x_list):
        df_LNprc['LNret2_' + var] = df_LNprc['LNprc_2_' + var] - df_LNprc.iloc[:,2+i]
    
    df_LNprc['LNret3'] = df_LNprc['LNprc_3']  - df_LNprc['LNprc_2_' + var]
    ...
    df_error = pd.DataFrame(columns=list(df_numerical_errors.columns),
                            data=[[Lead_VarDecomp_df["rdate"][0]] + num_errors]  # Wrap the row in an outer list
                            )           

    df_numerical_errors = pd.concat([df_numerical_errors,df_error])
    
    #-------------------------   Export Results ---------------------------
    df_results.to_stata(path + "/Output/Variance Decomposition Python/" + filename + "_IntermediaryReturns_Iterative.dta",
                        write_index=False)
    df_numerical_errors.to_csv(path + "/Output/Variance Decomposition Python/" + filename + "_IntermediaryReturns_Iterative_NumERRORS.csv")
