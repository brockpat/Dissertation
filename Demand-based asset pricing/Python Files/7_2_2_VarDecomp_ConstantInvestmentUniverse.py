...
Decomposition under a constant investment universe.

Only implemented for the Baseline Characteristics.
"""
...
#Import Functions for the Variance Decomposition
...
for year in range(2002, 2022):
    ...
    #---------------------------------------------------------------------#
    # 2) Restrict Data to Constant Investment Universes
    #---------------------------------------------------------------------#
    
    #---- Keep Managers who exist in both quarters. Moreover, within each Manager's Investment universe delete
    #       the non-overlapping stocks so that the investment universe is constant
    
    #Create one DataSet
    df = pd.concat([prev_VarDecomp_df, Lead_VarDecomp_df])
    
    # Step 1: Find the number of dates each (mgrno, permno) pair appears
    manager_stock_counts = df.groupby(['mgrno', 'permno'])['rdate'].nunique()
    
    #Step 2. Delete all instances where this combination doesn't appear twice
    consistent_stocks = manager_stock_counts[manager_stock_counts == 2]
    
    #Create a new DataFrame where each manager has a consistent investment universe
    df_consistent = df.merge(consistent_stocks.reset_index()[['mgrno', 'permno']], 
                     on=['mgrno', 'permno'], 
                     how='inner')
    
    #Split Data back into prevHoldingsData & HoldingsData
    
    prev_VarDecomp_df = df_consistent[df_consistent['rdate'].dt.year == year]
    #Sort dataframe (extremely important!)
        #If not sorted, then in getObjects() while iterating over managers and stocks the indices
        #might not refer to same manager or stock for the outputs
    prev_VarDecomp_df = prev_VarDecomp_df.sort_values(['mgrno','permno'])
    
    Lead_VarDecomp_df = df_consistent[df_consistent['rdate'].dt.year == year+1]
    #Sort dataframe (extremely important! --> see explanation for t-1 Holdings Data).
    Lead_VarDecomp_df = Lead_VarDecomp_df.sort_values(['mgrno','permno'])
    ...
    print("------------------------------------------- \n" + 
          "        Computing Baseline Price" + "\n"
          + "-------------------------------------------")
    #Due to using the constant investment universe, Managers and Stocks in t are cut out so that the
    #current price might not be the market clearing price anymore. Therefore,
    #We have to recompute it 
    
    root_baseline = solve_MarketClearing(prev_p["LNprc"].to_numpy(), 
                                 prev_s['LNshrout'].to_numpy(), prev_x, prev_aumVec, prev_RegCoeffsMat, prev_EpsilonMat, prev_consMat)
    print("Root Baseline Price Approximation Error = " + str(np.linalg.norm(root_baseline.fun)))
    
    LNprc_prev = prev_p.copy()
    LNprc_prev.loc[:, 'LNprc'] = root_baseline.x  
    LNprc_prev = LNprc_prev.rename(columns = {'LNprc':'LNprc_prev'})
    
    
    #---------------------------------------------------------------------#
    # 4) Lead supply, characteristics, aum, beta coefficients and compute
    ...
    #---------------------------------------------------------------------#
    # 5)  Compute Annual & Intermediary Returns and Save Final Returns
    #---------------------------------------------------------------------#
    ...
    #------------ Compute Annual Return & Assemble Final Dataframe ------------
    ...
    df_LNprc = df_LNprc.merge(StocksQ[['date','permno','LNretdA']], 
                              left_on = ['rdate','permno'], right_on = ['date','permno'], 
                              suffixes = ("","")).drop("date",axis=1)
    
    #Compute Constant Investment Universe Annual Return (sum of intermediary returns)
    df_LNprc['LNretA'] =  (df_LNprc["LNprc_6"] + df_LNprc["LNcfac_Lead"]  
                           - (df_LNprc["LNprc_prev"] + df_LNprc["LNcfac_prev"]) 
                           + df_LNprc['LNretdA']
                           )
    ...
    #-------------------------   Export Results ---------------------------
    df_results.to_stata(path + "/Output/Variance Decomposition Python/" + filename + "_IntermediaryReturns_ConstantInvUniverse.dta",
                        write_index=False)
    df_numerical_errors.to_csv(path + "/Output/Variance Decomposition Python/" + filename + "_IntermediaryReturns_ConstantInvUniverse_NumERRORS.csv")
